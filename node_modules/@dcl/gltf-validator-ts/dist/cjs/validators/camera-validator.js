...
exports.CameraValidator = void 0;
...
class CameraValidator {
    validate(camera, index) {
        ...
        else if (!Object.values(types_1.CameraType).includes(camera.type)) {
            messages.push({
                code: "VALUE_NOT_IN_LIST",
                message: "Invalid value '" +
                    camera.type +
                    "'. Valid values are ('orthographic', 'perspective').",
        ...
        if (camera.type === "perspective" || camera.perspective) {
            if (!camera.perspective) {
                ...
                if (camera.perspective.yfov === undefined) {
                    ...
                    if (!isFinite(camera.perspective.yfov) ||
                        isNaN(camera.perspective.yfov)) {
                        const valueStr = isNaN(camera.perspective.yfov)
                            ? "NaN"
                            : camera.perspective.yfov === Infinity
                                ? "Infinity"
                                : camera.perspective.yfov === -Infinity
                                    ? "-Infinity"
                ...
                if (camera.perspective.znear === undefined) {
                    ...
                    if (!isFinite(camera.perspective.znear) ||
                        isNaN(camera.perspective.znear)) {
                        const valueStr = isNaN(camera.perspective.znear)
                            ? "NaN"
                            : camera.perspective.znear === Infinity
                                ? "Infinity"
                                : camera.perspective.znear === -Infinity
                                    ? "-Infinity"
                ...
                if (camera.perspective.aspectRatio !== undefined) {
                    if (typeof camera.perspective.aspectRatio === "number") {
                        if (!isFinite(camera.perspective.aspectRatio) ||
                            isNaN(camera.perspective.aspectRatio)) {
                            const valueStr = isNaN(camera.perspective.aspectRatio)
                                ? "NaN"
                                : camera.perspective.aspectRatio === Infinity
                                    ? "Infinity"
                                    : camera.perspective.aspectRatio === -Infinity
                                        ? "-Infinity"
        ...
        if (camera.type === "orthographic" || camera.orthographic) {
            if (!camera.orthographic) {
                ...
                if (camera.orthographic.xmag === undefined) {
                    ...
                    if (!isFinite(camera.orthographic.xmag) ||
                        isNaN(camera.orthographic.xmag)) {
                        const valueStr = isNaN(camera.orthographic.xmag)
                            ? "NaN"
                            : camera.orthographic.xmag === Infinity
                                ? "Infinity"
                                : camera.orthographic.xmag === -Infinity
                                    ? "-Infinity"
                ...
                if (camera.orthographic.ymag === undefined) {
                    ...
                    if (!isFinite(camera.orthographic.ymag) ||
                        isNaN(camera.orthographic.ymag)) {
                        const valueStr = isNaN(camera.orthographic.ymag)
                            ? "NaN"
                            : camera.orthographic.ymag === Infinity
                                ? "Infinity"
                                : camera.orthographic.ymag === -Infinity
                                    ? "-Infinity"
                ...
                if (camera.orthographic.zfar === undefined) {
                    ...
                    if (!isFinite(camera.orthographic.zfar) ||
                        isNaN(camera.orthographic.zfar)) {
                        const valueStr = isNaN(camera.orthographic.zfar)
                            ? "NaN"
                            : camera.orthographic.zfar === Infinity
                                ? "Infinity"
                                : camera.orthographic.zfar === -Infinity
                                    ? "-Infinity"
                ...
                if (camera.orthographic.znear === undefined) {
                    ...
                    if (!isFinite(camera.orthographic.znear) ||
                        isNaN(camera.orthographic.znear)) {
                        const valueStr = isNaN(camera.orthographic.znear)
                            ? "NaN"
                            : camera.orthographic.znear === Infinity
                                ? "Infinity"
                                : camera.orthographic.znear === -Infinity
                                    ? "-Infinity"
...
exports.CameraValidator = CameraValidator;
