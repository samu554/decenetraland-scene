...
const gltf_validator_1 = require("./validators/gltf-validator");
const glb_validator_1 = require("./validators/glb-validator");
...
async function validateBytes(data, options = {}) {
    ...
    if (!detectedFormat) {
        ...
        try {
            await glb_validator_1.GLBValidator.parseGLB(data);
            detectedFormat = 'glb';
        }
        catch {
            // If GLB parsing fails completely, try GLTF parsing
            try {
                const parser = new parser_1.GLTFParser();
                parser.parse(data);
                detectedFormat = 'gltf';
            }
            catch {
    ...
    if (detectedFormat === 'glb') {
        try {
            const glbResult = await glb_validator_1.GLBValidator.parseGLB(data);
            gltf = glbResult.gltf;
    ...
    const validator = new gltf_validator_1.GLTFValidator(validatorOptions);
    ...
    const result = {
        ...
        validatorVersion: version_1.VALIDATOR_VERSION,
...
function hasMorphTargets(gltf) {
    if (!gltf.meshes)
        return false;
    return gltf.meshes.some(mesh => mesh && mesh.primitives && Array.isArray(mesh.primitives) &&
        mesh.primitives.some(primitive => primitive && primitive.targets && primitive.targets.length > 0));
}
            ...
            for (const primitive of mesh.primitives) {
                if (primitive) {
                    if (primitive.indices !== undefined) {
                        ...
                        if (accessor && primitive.mode === types_1.PrimitiveMode.TRIANGLES) {
                        ...
                        if (primitive.attributes && primitive.attributes['POSITION'] !== undefined) {
                            const accessor = gltf.accessors[primitive.attributes['POSITION']];
                            if (accessor && (primitive.mode === types_1.PrimitiveMode.TRIANGLES || primitive.mode === undefined)) {
                                total += Math.floor(accessor.count / 3);
                            }
                        }
                    }
