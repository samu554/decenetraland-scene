...
async function main(options) {
    ...
    if (willCreateRealm && !options.args['--baseUrl']) {
        throw new error_1.CliError('EXPORT_STATIC_BASE_URL_REQUIRED', i18next_1.default.t('errors.export_static.base_url_required'));
    }
    /* istanbul ignore if */
    if (willCreateRealm && !/^[a-z][a-z0-9-/]*$/i.test(options.args['--realmName'])) {
        throw new error_1.CliError('EXPORT_STATIC_INVALID_REALM_NAME', i18next_1.default.t('errors.export_static.invalid_realm_name'));
    }
    await fs.mkdir(outputDirectory, { recursive: true });
    /* istanbul ignore if */
    if (!(await fs.directoryExists(outputDirectory))) {
        throw new error_1.CliError('EXPORT_STATIC_INVALID_OUTPUT_DIRECTORY', i18next_1.default.t('errors.export_static.invalid_output_directory', { outputDirectory }));
    }
    const scenesUrn = [];
    const entities = [];
    ...
    if (willCreateRealm) {
        ...
        if (!(await fs.directoryExists(realmDirectory))) {
            throw new error_1.CliError('EXPORT_STATIC_INVALID_OUTPUT_DIRECTORY', i18next_1.default.t('errors.export_static.invalid_output_directory', { outputDirectory: realmDirectory }));
        }
